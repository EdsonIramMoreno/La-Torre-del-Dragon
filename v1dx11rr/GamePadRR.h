...
public:
	...
	void GamePadMovment(DXRR* dxrr) {
		float grados = (float)GetState().Gamepad.sThumbRX / 32767.0;

		if (grados > 0.19 || grados < -0.19) dxrr->izqder = grados / 15;

		grados = (float)GetState().Gamepad.sThumbRY / 32767.0;

		if (grados > 0.19 || grados < -0.19) dxrr->arriaba = grados / 15;


		float velocidad = (float)GetState().Gamepad.sThumbLY / 32767.0;
		if (velocidad > 0.19 || velocidad < -0.19) {
			if (GetState().Gamepad.wButtons & XINPUT_GAMEPAD_RIGHT_SHOULDER)
				velocidad *= 14.5;
			else if (GetState().Gamepad.wButtons & XINPUT_GAMEPAD_LEFT_SHOULDER) velocidad /= 3;
			else velocidad *= 2.5;
